...
static inline void print_load(int reg_idx,int reg_offset){
...
static inline void print_store(int reg_idx,int reg_offset){
  if (reg_offset >= -2048 && reg_offset <= 2047){
...
// 访问指令
Reg Visit(const koopa_raw_value_t &value){
  koopa_raw_value_t before_value = cur_value;
  cur_value = value;
  ...
  if (value_reg_map.count(value)){
    if (value_reg_map[value].reg_idx == -1){
      ...
      print_load(reg_idx,reg_offset);
    }
    cur_value = before_value;
  ...
  case KOOPA_RVT_ALLOC:
    ...
    stack_top += 4;//目前你的编译器只会生成 alloc i32, 所以应为其分配的内存大小为 4 字节.
    value_reg_map[value] = res_reg;
    break;
  case KOOPA_RVT_LOAD:
...
void Visit(const koopa_raw_return_t &ret){
  ...
  if (ret_value){
    ...
    if (res_reg.reg_idx != A0)
        std::cout << "\tmv    a0, " << register_name[res_reg.reg_idx] << std::endl;
  }
  //函数返回前, 即 ret 指令之前, 你需要生成复原栈指针的指令, 将栈指针加上S'
...
Reg Visit(const koopa_raw_load_t &load){
  ...
  if (value_reg_map[src].reg_idx != -1){
...
void Visit(const koopa_raw_store_t &store){
  ...
  print_store(reg_idx,reg_offset);
}
